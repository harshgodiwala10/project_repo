...
class CSVPreprocessor:
    def __init__(self, root):
        ...
        self.bg_image = Image.open("background.jpg")  # Use your own image file
        self.bg_image = self.bg_image.resize((1200, 800), Image.Resampling.LANCZOS)
        self.bg_photo = ImageTk.PhotoImage(self.bg_image)

        self.bg_label = ctk.CTkLabel(root, image=self.bg_photo, text="")
        self.bg_label.place(x=0, y=0, relwidth=1, relheight=1)  # Full-screen background

        # Transparent Title
        self.title_label = ctk.CTkLabel(
            root, text="CSV Data Preprocessing Tool",
            font=("Helvetica", 30, "bold"), text_color="white"
        )
        self.title_label.pack(pady=20)

        # Buttons Frame (with transparency)
        self.frame_buttons = ctk.CTkFrame(root, fg_color="rgba(34, 34, 34, 150)")  # Transparent dark frame
        self.frame_buttons.pack(fill='x', padx=20, pady=10)

        ctk.CTkButton(self.frame_buttons, text="Upload CSV", command=self.load_csv, width=200, fg_color="#4CAF50",
                      hover_color="#45A049").pack(side="left", padx=15)
        ctk.CTkButton(self.frame_buttons, text="Preprocess Data", command=self.process_data, width=200,
                      fg_color="#2196F3", hover_color="#1E88E5").pack(side="left", padx=15)
        ctk.CTkButton(self.frame_buttons, text="Exit", command=root.quit, width=200, fg_color="#F44336",
                      hover_color="#D32F2F").pack(side="left", padx=15)

        # Data Display Frame
        self.frame_data = ctk.CTkFrame(root, fg_color="rgba(51, 51, 51, 150)")  # Semi-transparent background
        self.frame_data.pack(fill='both', expand=True, padx=20, pady=10)

        # Column Selection Checkboxes
        self.frame_columns = ctk.CTkFrame(root, fg_color="rgba(68, 68, 68, 180)")
        self.frame_columns.pack(fill='x', padx=20, pady=10)
        ctk.CTkLabel(self.frame_columns, text="Select Columns for Preprocessing:", font=("Arial", 16),
